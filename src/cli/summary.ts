...
function formatGroupLines(groups: CliSummaryGroup[]): string[] {
  return groups.map(group => `  ${group.name}: ${group.sessions} sessions, ${group.requests} requests`);
}
...
export function formatTextSummary(summary: CliSummary): string {
  const lines = [
    'AI Engineer Coach CLI Summary',
    '',
    `Generated: ${summary.generatedAt}`,
    `Sessions: ${summary.totals.sessions}`,
    `Requests: ${summary.totals.requests}`,
    `Workspaces: ${summary.totals.workspaces}`,
    `Edited files: ${summary.totals.editedFiles}`,
    `Tool calls: ${summary.totals.tools}`,
    `AI lines of code: ${summary.totals.aiLinesOfCode}`,
    `Prompt tokens: ${summary.totals.promptTokens}`,
    `Completion tokens: ${summary.totals.completionTokens}`,
    ...
    '',
    'Top tools:',
    ...(summary.topTools.length > 0 ? formatItemLines(summary.topTools) : ['  none']),
    '',
    'Top edited files:',
    ...(summary.topEditedFiles.length > 0 ? formatItemLines(summary.topEditedFiles) : ['  none']),
  ];
  return `${lines.join('\n')}\n`;
}
...
export function formatMarkdownSummary(summary: CliSummary): string {
  const rows = [
    ...
    '## By Harness',
    '',
    '| Harness | Sessions | Requests |',
    '| --- | ---: | ---: |',
    ...markdownGroupRows(summary.byHarness),
    '',
    '## Top Workspaces',
    '',
    '| Workspace | Sessions | Requests |',
    '| --- | ---: | ---: |',
